...
describe("モック認証システム", () => {
  ...
  describe("mockAmplifyConfig", () => {
    it("モックのAmplify設定を持つ", () => {
      ...
      expect(mockAmplifyConfig.Auth.Cognito).toHaveProperty(
        "userPoolId",
        "mock-user-pool",
      );
      expect(mockAmplifyConfig.Auth.Cognito).toHaveProperty(
  ...
  describe("mockUser", () => {
    it("デフォルトのモックユーザー情報を持つ", () => {
      ...
      expect(mockUser.signInDetails).toHaveProperty(
        "loginId",
        "dev@example.com",
      );
    });
  ...
  describe("mockAuth.signIn", () => {
    ...
    it("モック認証が無効な場合、エラーがスローされる", async () => {
      ...
      await expect(
        mockAuth.signIn({ username: "test", password: "pass" }),
  ...
  describe("mockAuth.getCurrentUser", () => {
    ...
    it("未認証の場合、エラーがスローされる", async () => {
      ...
      await expect(mockAuth.getCurrentUser()).rejects.toThrow(
        "Not authenticated",
      );
    });
  ...
  describe("mockAuth.signOut", () => {
    ...
    it("モック認証が無効な場合、エラーがスローされる", async () => {
      ...
      await expect(mockAuth.signOut()).rejects.toThrow("Mock auth not enabled");
    });
  });
});
